...
const imageUrl = "/Img/music/images/play/"
let song_image, song_file, song_lyrics, song_name, song_singer,background_img;
songId = document.getElementById("music-title").dataset.songId;
fetch('/song/api/'+songId)
    .then(response => response.json())
    .then(data => {
        data.song_singer = undefined;
        data.song_lyrics = undefined;
        data.song_name = undefined;
        data.song_file = undefined;
        data.song_image = undefined;
        data.background_img = undefined;
        song_image = data.song_image;
        song_file = data.song_file;
        song_lyrics = data.song_lyrics;
        song_name = data.song_name;
        song_singer = data.song_singer;
        background_img = data.backgroundImage
        console.log(data);
    })
    .catch((error) => {
...
// 总进度条
let progressTotal = document.getElementById('progress-total');
...
// 播放模式按钮
let mode = document.getElementById('playMode');
// 上一首
...
// 音量调节滑块
let volumeTogger = document.getElementById('volumn-togger');
...
progressTotal.addEventListener('mousedown', function (event) {
    ...
    if (!audio.paused || audio.currentTime != 0) {
        let pgsWidth = parseFloat(window.getComputedStyle(progressTotal, null).width.replace('px', ''));
...
function initMusic() {
    ...
    audio.ondurationchange = function () {
        musicTitle.innerText = song_name;
        author.innerText = song_singer;
        recordImg.style.backgroundImage = song_image;
        body.style.backgroundImage = background_img;
...
mode.addEventListener('click', function (event) {
    ...
    mode.style.backgroundImage = background_img;
});

// audio.onended = function () {
//     if (modeId == 2) {
//         // 跳转至下一首歌
//         musicId = (musicId + 1) % song_count;
//     }
//     else if (modeId == 3) {
//         // 随机生成下一首歌的序号
//         let oldId = musicId;
...
//     // 向新窗口传值
//     let storage_list = window.sessionStorage;
...
function setNoVolumn() {
    if (volumeTogger.value == 0) {
        ...
        volume.style.backgroundImage = "url(" + imageUrl + "voice.png')";
    }
    else {
        lastVolumn = volumeTogger.value;
        volumeTogger.value = 0;
        volume.style.backgroundImage = "url("+imageUrl+"mute.png')";
    }
}
